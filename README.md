...
## Migration guide from Artemis to graphql_codegen

1) Install graphql_codegen deps listed [here](https://github.com/heftapp/graphql_codegen/tree/main/packages/graphql_codegen#installation)
2) Add `graphql_codegen:` field to `build.yaml` as shown [here](https://github.com/heftapp/graphql_codegen/tree/main/packages/graphql_codegen#options) and any options that you might need. [These](https://github.com/baconcheese113/graphql_codegen_example/blob/master/build.yaml) might be good starter options for your project. I'd recommend omitting `scalars` before the first build so you can see which scalar types you need to enter from the logs. `schema_mapping` will go away completely and become `outputDirectory` and `assetsPath`. Helpers are generated by default, but selected based on your `clients` setting. 
3) Update `.graphqlconfig` to reflect where your schema will live and add the following to `analysis_options.yaml` to turn off linting the generated files
```yaml
analyzer:
  exclude:
    - lib/**/__generated__/* # or wherever you're storing your generated files
```
4) Update schema. Then add the `scalars` that the logs are telling you need to be added. Delete your `coercers.dart` file if you no longer need it.
5) Refactor all previous usages, normally in the pattern shown [here](https://github.com/baconcheese113/handle-it-app/commit/8e69da001ee543b566254ab534596baba6d6a6c6#diff-2c2e0c9d881d8ed390d26668f63749694d75db1a99db227b4531d471774d2346) You can also remove any `__typename` fields from queries if you want because it's automatically added with graphql_codegen, also you can get rid of any widget param casts or graphql aliases only to get Artemis to work correctly
6) At some point delete the old schema api files to help weed out any remaining references
7) Remove Artemis from `build.yaml` and `pubspec.yaml`
8) Profit
